 ...
 * things: the drop zone and the export menu. The third — the imported files
 * with their revocation — went with the population it described.
 *
 ...
 * a *row* gave three consecutive lines carrying three identical red *« Oublier
 * cet import (214) »* buttons: the subject of the gesture was the file,
 * repeating it on 214 rows made it read as a row gesture, and **somebody
 * deletes 214 events believing they are removing one**. So the gesture lived
 * here, once, beside its subject.
 *
 * A mounted file was a second truth and had to be revocable whole because its
 * rows could not be corrected; an uploaded file is a payload, its rows are
 * ordinary rows, and undoing an import is **the deletion on the ledger's own
 * reduction** (`BulkDelete`) — which is offered where the reduction is, names
 * it, counts its rows, and reaches the twelve events somebody mistyped as well.
 *
 * Two things about what is left are still decisions:
 *
 *  - **The export is four files, one of which is the reduction and one of which
 *    is not a backup at all** (#796, #836). It was *total or nothing* until
 *    #796, on the argument that a partial file is not a round trip and makes
 *    re-importing look like a restore. What settles it is the **name**: the
 *    server calls a reduction a selection and not a backup, so the file cannot
 *    replace the whole one on a disk — and the reduction is taken on the side
 *    that owns the importable form, so what comes back is an ordinary event
 *    file rather than an extract wearing one's clothes. The menu itself is
 *    `ExportMenu.tsx`.
