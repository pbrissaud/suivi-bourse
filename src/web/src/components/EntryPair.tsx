 ...
 * only one that exists to be mounted twice on purpose (#723, #726).
 *
 * It replaces the shape everything else on this page refuses: an empty table
 * with a small button over it.
 *
 ...
 *    disappearing. A pair that renders as one entry reads as a breakage, and
 *    the reader cannot tell an entrance this install happens to lack from a
 *    product that never offered it at all.
 *  - **it is not an `EmptyState`.** That primitive says *this is empty* in one
 *    sentence with at most one way out; here emptiness is not the news — the
 *    two ways out are.
 *
 * The first-run modal's last step is this component, mounted as it is (#726).
 * That is why it lives beside `Stat`, `EmptyState` and `Refusal` rather than
 * under `components/data/`: a second design of it is exactly what the criterion
 * forbids.
 *
 * `data-empty` is the reason `EmptyState` carries one — *this reader has
 * recorded nothing* is a claim about their own data, and never one to make on a
 * read in flight — and it is **the mount's, not the component's** (#726). The
 * first-run modal mounts this same pair while stating no emptiness at all: it
 * offers two doors before anything has been read. So the marker is a prop the
 * ledger sets and the modal does not, rather than a property travelling with
 * the component into a surface where it would be false.
 */
