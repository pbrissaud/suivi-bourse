 ...
 * a failed read is one of the reasons it carries, through `Unreadable`: the
 * band that used to announce failures at the top of the column is retired and
 * not replaced, so the sentence lives where the missing content would have
 * been.
 *
 * It is deliberately **not** `role="status"`. What announces the *installation*
 * is the bell, once.
 *
 * It carries a `data-empty` attribute for the same reason it carries no role:
 * the marker is for **a test**, not for a reader. Every page is replayed with
 * one of its reads left unresolved and the assertion is that no such marker
 * appears — *empty* being a claim about the reader's own data, and never one to
 * make on a request still in flight. An attribute is what says that to a query
 * selector without saying anything to a screen reader.
 *
