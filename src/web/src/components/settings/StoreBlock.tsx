 ...
 *    it go quiet while it was still true, and counting it would give a
 *    permanent badge.
 *  - **The size, with what a purge will not do.** Measured: 79 % of a real
 *    store's rows purged for **zero bytes** returned — 126,0 Mo before, 126,0
 *    Mo after, the same content rebuilt from scratch fitting in 26,0. Shown
 *    bare beside a purge button the figure is a lie by juxtaposition; hidden,
 *    it is still there for anyone who runs `du`, and only its explanation is
 *    gone.
 *  - **The last write of the ledger**, and never the last observed price. The
 *    second is liveness, which the bell answers (#829); here it would make a
 *    store whose last import was a year ago read as freshly written.
 *
 * **Its rows come from two reads, so it waits a row at a time** (#777). So it
 * is applied one notch lower, on the rows a read owns, the way #775 applied it
 * to the accounts table's `perf` cells: what a read has not answered yet is not
 * rendered at all, an em dash and a sentence being two ways of stating
 * something nobody has observed. The block waits **whole** in one state only,
 * neither read having landed.
 *
 * What is left here is the file.
 */
...
interface StoreBlockProps {
   ...
   * included (#777). The two facts above them come off
   ...
   * above the page to say so on the card's behalf.
   */
...
export function StoreBlock({ runtimeStore, store, failure = null }: StoreBlockProps) {
  ...
  // **Neither read has landed, so the block does not exist yet** (#777): a title over an empty frame is the hand-written skeleton this
