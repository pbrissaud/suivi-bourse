...
def _metrics(store, declare_ledger, events, accounts):
    ...
    return main.SuiviBourseMetrics(_CashConfigManager(store))
...
def test_no_portfolio_totals_across_two_currencies():
    """Two currencies -> no global series (pooling them would need FX).

    Asserted on the pure module rather than through the job, and the reason is
    the store: ``account`` declares ``id``/``type``/``label`` and nothing else,
    so every stored account reads back with ``currency = None`` until the
    reporting currency lands (#702). Faking a mixed-currency declaration through
    the wiring would be asserting on a shape the store cannot hold.
    """
    events = [
        Event(date(2024, 1, 1), EventType.DEPOSIT, amount=1000.0, account="PEA"),
        Event(date(2024, 1, 1), EventType.DEPOSIT, amount=500.0, account="CTO"),
    ]
    accounts = [Account("PEA", "PEA", "Mon PEA", "EUR"),
                Account("CTO", "CTO", "My CTO", "USD")]
    timeline = EventAggregator().replay(events)
    per_account = {
        a.id: performance.compute_account(
            timeline, a, set(), lambda s, d: None,
            date(2024, 1, 1), date(2024, 1, 2))
        for a in accounts
    }
    assert performance.compute_portfolio_total(
        timeline, accounts, set(), lambda s, d: None,
        date(2024, 1, 1), date(2024, 1, 2), per_account) is None
