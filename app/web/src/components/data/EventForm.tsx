...
export function EventForm({ open, event, accounts, onClose }: EventFormProps) {
  const { t } = useI18n()
  ...
  function submit() {
    ...
    const account = draft.account.trim() || single?.id || ''
    if (account === '') found.account = 'data.form.required'
