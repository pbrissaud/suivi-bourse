...
class SuiviBourseMetrics:
    ...
    def __init__(self, config_manager: ConfigurationManager,
        ...
        # workload that reads it (issue #847) — the scrape below, and the
        # backfill further down this class until #848 takes it its own way.
    ...
    # *this* object — by the suite, and by the backfill below for the cache — so
    # the class keeps a window open onto each rather than a second copy: a copy
    # would be a second answer to "how many times has this symbol failed", and
    # the #617 back-off is the one guard that cannot afford two. The ``info``
    # cache is here for the same reason and one more: the backfill reads it
    # through this window until #848 is handed it, so a rebind must reach the
    # scrape rather than leave it observing into an object nobody reads.
    ...
    def _learn_quote_currency(self, symbol: str) -> Tuple[Optional[str], bool]:
        ...
        * ``(None, True)`` — the request did not complete, or the write did not.
        ...
        self._share_info_cache.learned(symbol, info)
