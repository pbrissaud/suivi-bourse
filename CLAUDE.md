...
contract above. `website/crowdin.yml` is the **documentation** project's
configuration and is not the front's (#712): Markdown against ICU JSON, a
release's rhythm against a component's. Its sources are `docs/` plus the theme
catalogues under `i18n/en/` (generated by `pnpm write-translations`, committed);
