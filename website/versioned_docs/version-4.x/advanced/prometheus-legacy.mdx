...
[InfluxDB](/docs/v4/advanced/influxdb)).

## Configuration

| Variable | Default | Description |
|----------|---------|-------------|
| `SB_PROMETHEUS_ENABLED` | `true` | Mount the `/metrics` endpoint |
...
:::note What changed in v5
`/metrics` is now served by the app's own web server rather than a separate
HTTP server inside the exporter. Nothing changes for a scraper: the endpoint
keeps its own port and its own path, so existing `scrape_configs` need no edit.
Two consequences worth knowing: `SB_PROMETHEUS_ENABLED=false` now unmounts the
route instead of skipping a server (port `SB_METRICS_PORT` is then not bound at
all), and the container healthcheck no longer probes `/metrics` — it probes
`/health` on `SB_WEB_PORT`, so turning Prometheus off no longer leaves the
deployment without a real healthcheck.
:::
...
| `sb_price_staleness` | `1` when the stored price is **silently stale** — frozen across consecutive open-market cycles while the live quote moves — `0` otherwise. See the [price-freshness sonde](/docs/v4/advanced/market-aware-scraping#price-freshness-sonde). |
...
See [Upgrading to accounts](/docs/v4/advanced/upgrading-accounts).
:::

For opt-in [accounts](/docs/v4/configuration/events-mode#accounts-opt-in), extra
...
[InfluxDB 3 integration](/docs/v4/advanced/influxdb) — the Prometheus endpoint only
