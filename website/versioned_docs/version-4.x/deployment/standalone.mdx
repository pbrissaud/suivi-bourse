...
described in the [Design](/docs/v4/intro/design) section.

:::tip
If you don't already run InfluxDB and Grafana, prefer the
[Docker Compose](/docs/v4/deployment/docker-compose) stack.
:::
...
3. Write a `config.yaml` (manual mode) — or a `settings.yaml` + `events/`
   ...
   [configuration section](/docs/v4/category/configuration), in the SuiviBourse
   config directory:
...
1. Create a service unit file in `/etc/systemd/system/suivi-bourse.service`
   (replace `<user>`/`<group>` and the token):
    ...
    ExecStart=uv run gunicorn -c src/gunicorn.conf.py 'web:create_app()'
    User=<user>
    Group=<group>
...
See the [configuration overview](/docs/v4/configuration/overview#environment-variables)
for the full list.
...
- [`docker-compose/grafana_provisioning/dashboards/suivi_bourse.json`](https://github.com/pbrissaud/suivi-bourse/blob/v4.2.2/docker-compose/grafana_provisioning/dashboards/suivi_bourse.json)

An example datasource definition is available in
[`grafana_provisioning/datasources/all.yml`](https://github.com/pbrissaud/suivi-bourse/blob/v4.2.2/docker-compose/grafana_provisioning/datasources/all.yml).

:::tip Prefer Prometheus?
If you still rely on a Prometheus/Grafana stack, the app keeps exposing a legacy
`/metrics` endpoint. See [Legacy Prometheus endpoint](/docs/v4/advanced/prometheus-legacy).
:::
