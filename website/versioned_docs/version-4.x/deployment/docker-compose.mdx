...
Edit `data/config.yaml` following the [manual mode doc](/docs/v4/configuration/manual-mode):
...
[events mode doc](/docs/v4/configuration/events-mode) for the full CSV format and
[backfill](/docs/v4/backfill) for historical data.
...
| App | `SB_WEB_PORT` | `8080` | Web API and `/health` (the healthcheck's target) |
...
The stack mounts **one** directory at `/home/appuser/.config/SuiviBourse` inside
the app container:
...
### Who owns the files {#config-ownership}

The mount is **writable** — the app edits your event files and the `accounts:`
block. So it has to run as whoever owns them, or every file it writes comes back
owned by uid 1000 and you can no longer edit your own portfolio by hand.
`make init` records that for you:
...
Leave them out and the container runs as `1000:1000`, which is right only if
that is your uid — check with `id -u` / `id -g`. On a platform that generates
its own environment (Coolify, Dokploy), set the two yourself to match the owner
of `SB_CONFIG_DIR` on the host.
...
| `SB_UID` / `SB_GID` | `1000` | Uid/gid the app runs as — see [Who owns the files](#config-ownership) |
...
| `SB_WEB_PORT` | `8080` | Port for the web API and `/health` |
