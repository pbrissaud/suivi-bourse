...
Events mode is also what unlocks [historical backfill](/docs/v4/backfill): because
...
[mode selection](/docs/v4/configuration/overview#mode-selection-priority).
:::
...
your event files and `settings.yaml` alike:

- If nothing changed → the cached, already-aggregated data is reused.
- If anything changed → a full reload and re-aggregation happens.

Because `settings.yaml` is watched too, editing the `accounts:` block takes
effect on the next ingestion cycle. `mode`, `events.source` and `events.watch`
are read once at startup and still need a restart — they describe how the
deployment is wired, not what is in your portfolio.
