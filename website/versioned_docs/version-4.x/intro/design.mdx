...
2. Independent scheduled jobs run in parallel:
   - **Scraping** — fetches current prices from Yahoo! Finance. There is **one
     ...
     [market-aware](/docs/v4/advanced/market-aware-scraping): it polls only while
   ...
   - **Backfill** — progressively fills in historical prices, in
     [both directions](/docs/v4/backfill): backward to your first purchase, and
...
[market-aware scraping](/docs/v4/advanced/market-aware-scraping) for the full model,
...
- **InfluxDB 3 Core** is the primary datastore. All live and historical points
  ...
  [InfluxDB data model](/docs/v4/advanced/influxdb) for the full tag/field schema.
...
[Configuration](/docs/v4/category/configuration) section for details.
