...
| `INFLUXDB_TOKEN` | _(preset)_ | InfluxDB admin token — **required** |
| `LOG_LEVEL` | `INFO` | Logging level |

:::caution InfluxDB token
The stack ships with a **development** token
(`apiv3_suivi-bourse-admin-token-for-local-development`). **Change it before any
non-local use** — it is now a single edit, in `.env`:
...
The stack is published to the registry alongside each release, and Compose ≥ 2.32
can pull it straight from there — no download-and-extract step at all:
...
config directory in the working directory — `SB_CONFIG_DIR` points at it.
