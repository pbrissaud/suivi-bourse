...
want to know *why* a part of the product is the way it is and neither
`CONTEXT.md` (the vocabulary) nor `docs/adr/` (the structural decisions) says so.

What stays alive elsewhere:

- `CLAUDE.md` at the root — the broad lines and the commands;
- `app/CLAUDE.md`, `app/web/CLAUDE.md`, `website/CLAUDE.md` — the detail per
  sub-tree, loaded only when you work there;
- `docs/adr/` — the 26 structural decisions;
- `CONTEXT.md` — the domain glossary;
- GitHub Issues — the tracker.

Nothing here is kept up to date. Where it contradicts an ADR or the code, the
code and the ADR win.
