...
def test_a_bare_date_bounds_the_window_in_utc_and_keeps_its_first_day(tmp_path):
    ...
    then lands on a `DATE` column, where `src/application/CLAUDE.md` names the second trap:
...
def test_a_type_is_read_however_it_is_spelled(tmp_path):
    ...
    (``api.api._event_from_body``), and two spellings of one rule eventually
...
def _serve_bundle_from(monkeypatch, directory):
    ...
    monkeypatch.setattr(api_module, '_static_dir', lambda: directory)
...
def test_the_mount_observation_survives_an_unreadable_store(tmp_path):
    ...
    client = build_client(
        tmp_path, break_store=True,
        accounts=ACCOUNTS_FILE, events=ACCOUNTS_EVENTS)
    api_module.current_runtime().store_persistence = 'ephemeral'
...
def test_saving_a_dial_takes_effect_with_no_restart(tmp_path):
    ...
    assert api_module.current_runtime().workloads.backfill_chunk_days == 90
...
def test_the_answer_quantifies_what_the_change_reached(tmp_path):
    ...
    assert api_module.current_runtime().workloads.rearm_calls == 1
...
def test_reposting_the_same_value_re_arms_nothing(tmp_path):
    ...
    assert api_module.current_runtime().workloads.rearm_calls == 0
...
def test_what_the_reconstruction_withholds_cannot_be_acknowledged(tmp_path):
    ...
    client, opened = build_client_and_store(tmp_path)
    _cash_heavy_account(opened)
    api_module.current_runtime().workloads.reconstruction = (1, 2)
