"""What the tree holds as a whole — the subject here is the tree itself, never
a module of it."""
...
#: The Python the product is made of: two packages under one import root,
#: `src/application` (the app) and `src/api` (Flask). They are named rather than
#: reached through `src/`, because `src/web` — the front — lives there too and is
#: not Python: walking `src/` would walk `node_modules`, and a dependency's own
#: `.py` files are nobody's convention to police.
_SOURCE = (_ROOT / 'src' / 'application', _ROOT / 'src' / 'api')

#: The suite, policed beside the product wherever the rule has two halves.
_SUITE = (_ROOT / 'tests',)


def _python_files(roots) -> list:
    """Every `.py` under the given roots, sorted, this file excepted."""
...
def test_the_tree_reads_one_clock_and_it_is_the_products():
    ...
    The two source packages are policed beside `tests/`: the rule names the product as
    ...
    """
    offenders = []
    for roots in (_SOURCE, _SUITE):
        scanned = _python_files(roots)
        ...
        assert len(scanned) > 20, roots
        offenders += [path.relative_to(_ROOT).as_posix() for path in scanned
                      if _LOCAL_CLOCK.search(path.read_text())]

    assert offenders == []


#: The **repair** of a naive instant, written under a private name. `src/application/`
...
#: at the market's edge, and `api.api._parse_instant` repairs an ISO string
...
_INSTANTS = 'src/application/instants.py'
...
def test_the_repair_of_an_instant_is_written_once_in_the_tree():
    ...
    *whatever it is called*: enumerating names is not the rule, and the first
    ...
    home = (_ROOT / _INSTANTS).read_text()
    assert 'def utc(' in home and 'def iso(' in home
    importers = [path.relative_to(_ROOT).as_posix() for path in scanned
...
def test_the_pure_modules_are_pure_at_the_import():
    ...
    program = (
        ...
        'import importlib\n'
        'for name in %r: importlib.import_module("application." + name)\n'
        'print(",".join(sorted(m for m in sys.modules '
        '                      if m.split(".")[0] in %r)))\n'
    ) % (str(_ROOT / 'src'), _PURE, _HEAVY)
...
_MARKET = 'src/application/market.py'
...
def _offenders(roots, pattern):
    """The files of one half of the tree whose text matches, this one excepted."""
    scanned = _python_files(roots)
    # Asserted per half, for the reason written on the clock guard above: a
    # root that stopped resolving reads no file and reports clean for ever.
    assert len(scanned) > 20, roots
    return sorted(path.relative_to(_ROOT).as_posix() for path in scanned
...
def test_the_market_edge_is_imported_in_one_module():
    ...
    `tests/` is policed beside the two source packages, because the suite's one faked
    ...
    """
    binders = (_offenders(_SOURCE, _BINDS_YFINANCE)
               + _offenders(_SUITE, _BINDS_YFINANCE))
    assert binders == [_MARKET]

    # And in the product, the exception classes come in at the same door.
    assert _offenders(_SOURCE, _READS_FROM_YFINANCE) == [_MARKET]
...
def test_the_sentinel_is_written_nowhere_at_all():
    ...
    """
    assert _offenders(_SOURCE, _NAMES_THE_SENTINEL) == []
    assert _offenders(_SUITE, _NAMES_THE_SENTINEL) == []
...
def test_no_class_in_the_product_outgrows_a_reading():
    ...
    *holds*, though: a fifth concern written back into one of the four, or a
    ...
    """
    oversized = {}
    for path in _python_files(_SOURCE):
